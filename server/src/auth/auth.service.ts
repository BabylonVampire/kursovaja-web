...
export class AuthService {
  ...
  async signUpLocal(
    ...
    try {
      ...
      if (newUser instanceof HttpException) {
        return new HttpException('Не удалось создать пользователя', HttpStatus.UNAUTHORIZED);
      }
      ...
      console.error(error);
      return new HttpException('Ошибка при регистрации', HttpStatus.INTERNAL_SERVER_ERROR, {
        cause: error,
      });
    }
  }

  async signInLocal(authDto: AuthDto): Promise<{ tokens: Tokens; user: User } | HttpException> {
    try {
      ...
      if (!passwordMatches) {
        return new HttpException('Указан неверный пароль', HttpStatus.UNAUTHORIZED);
      }
      ...
      console.log(error);
      return new HttpException('Ошибка при входе', HttpStatus.INTERNAL_SERVER_ERROR, {
        cause: error,
      });
    }
  }
  ...
  async logout(userId: string): Promise<LogoutResponse | HttpException> {
    try {
      ...
      await user.update({ hashedRT: undefined });
      return { message: 'Выход из системы совершён успешно' };
    } catch (error) {
      return new HttpException('Ошибка при выходе из системы', HttpStatus.INTERNAL_SERVER_ERROR, {
        cause: error,
      });
    }
  }
